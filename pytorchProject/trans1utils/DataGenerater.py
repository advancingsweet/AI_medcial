...
def data_with_loader(vocab,type_ = 'train',fold = None,hla_max_len=34,pep_max_len=15,batch_size = 1024):
    data = None
    if type_ == 'train':
        data = pd.read_csv('../pytorchProject/data/train_data_fold{}.csv'.format(fold), index_col = 0)
    elif type_ == 'val':
        data = pd.read_csv('../pytorchProject/data/val_data_fold{}.csv'.format(fold), index_col = 0)
...
def hla_pep_data_loader(file_name,vocab, hla_max_len=34, pep_max_len=15, batch_size=1024, train_rate=0.8):
    data = pd.read_csv("../pytorchProject/data/{}.csv".format(file_name))
