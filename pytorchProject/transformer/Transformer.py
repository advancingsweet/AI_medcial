...
from trans1utils.Criterion import *
from transblock.Embedding import Embedding
from transblock.Encoder import Encoder
from transblock.Decoder import Decoder


class Transformer(nn.Module):
    def __init__(self,vocab_size,d_model=64,n_enc_layers=1,n_enc_heads=1,n_dec_layers=1,n_dec_heads=1,d_k=64,d_v=64,d_ff=512,hla_pep_concat_len=49,pep_tcr_concat_len=39,dropout_rate=0.1,max_len=5000,device = torch.device("cuda:0")):
        super(Transformer, self).__init__()
        self.device = device

        # Embedding block:
        self.hla_embedding = Embedding(vocab_size,d_model,dropout_rate,max_len).to(self.device)
        self.pep_embedding = Embedding(vocab_size,d_model,dropout_rate,max_len).to(self.device)
        self.tcr_embedding = Embedding(vocab_size,d_model,dropout_rate,max_len).to(self.device)

        # Encoder block:
        self.hla_encoder = Encoder(n_enc_layers,n_enc_heads,d_model,d_k,d_v,d_ff,device).to(self.device)
        self.pep_encoder = Encoder(n_enc_layers,n_enc_heads,d_model,d_k,d_v,d_ff,device).to(self.device)
        self.tcr_encoder = Encoder(n_enc_layers,n_enc_heads,d_model,d_k,d_v,d_ff,device).to(self.device)

        # Decoder block:
        self.hla_pep_decoder = Decoder(n_dec_layers,n_dec_heads,d_model,d_k,d_v,d_ff,dropout_rate,max_len,device).to(self.device)
        self.pep_tcr_decoder = Decoder(n_dec_layers,n_dec_heads,d_model,d_k,d_v,d_ff,dropout_rate,max_len,device).to(self.device)
    ...
    def forward(self, first_inputs,second_inputs,num_group):
        ...
        if num_group == 1:
            ...
            2) hla_self_attentions:     [n_layers,batch_size,n_heads,hla_max_len,hla_max_len]
            1) pep_enc_outputs:         [batch_size,pep_max_len,d_model]
            2) pep_self_attentions:     [n_layers,batch_size,n_heads,pep_max_len,pep_max_len]
            '''
            hla_enc_outputs, hla_self_attentions = self.hla_encoder(hla_inputs)
            pep_enc_outputs, pep_self_attentions = self.pep_encoder(pep_inputs)
            ...
            2) hla_pep_concat_self_attentions:  [n_layers,batch_size,n_heads,hla_pep_concat_len,hla_pep_concat_len]
            '''
            dec_hla_pep_concat_outputs, hla_pep_concat_self_attentions = self.hla_pep_decoder(dec_hla_pep_concat_inputs)
            ...
            return (
                hla_pep_proj_outputs,           # [batch_size,2]
                hla_self_attentions,            # [n_layers,batch_size,n_heads,hla_max_len,hla_max_len]
                pep_self_attentions,            # [n_layers,batch_size,n_heads,peptide_max_len,peptide_max_len]
                hla_pep_concat_self_attentions, # [n_layers,batch_size,n_heads,hla_pep_concat_len,hla_pep_concat_len]
            )
        else:
            '''
            1.Embedding block:
            ...
            2) tcr_self_attentions:     [n_layers,batch_size,n_heads,tcr_max_len,tcr_max_len]
            '''
            pep_enc_outputs, pep_self_attentions = self.pep_encoder(pep_inputs)
            tcr_enc_outputs, tcr_self_attentions = self.tcr_encoder(tcr_inputs)
            ...
            2) pep_tcr_concat_self_attentions:  [n_layers,batch_size,n_heads,pep_tcr_concat_len,pep_tcr_concat_len]
            '''
            dec_pep_tcr_concat_outputs, pep_tcr_concat_self_attentions = self.pep_tcr_decoder(dec_pep_tcr_concat_inputs)
            ...
            return (
                ...
                pep_self_attentions,            # [n_layers,batch_size,n_heads,pep_max_len,pep_max_len]
                tcr_self_attentions,            # [n_layers,batch_size,n_heads,tcr_max_len,tcr_max_len]
                pep_tcr_concat_self_attentions, # [n_layers,batch_size,n_heads,pep_tcr_concat_len,pep_tcr_concat_len]
            )
    ...
    def train_per_epoch(self,train_loader,epoch,epochs,criterion,optimizer,num_group=1,threshold=0.5,metrics_print_=True):
        ...
        for train_first_inputs, train_second_inputs, train_labels in tqdm(train_loader):
            ...
            # train_concat_self_attentions: [n_layers,batch_size,n_heads,concat_len,concat_len]
