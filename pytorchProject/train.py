...
for n_layers in range(1, 10):
    for n_heads in range(1, 6):
        for fold, (train_index, val_index) in enumerate(kf.split(train_pep_tcr_loader.dataset)):
            ...
            train_pep_tcr \
                = Data.DataLoader(PEP_TCR_DataSet(train_pep_tcr_set[0], train_pep_tcr_set[1], train_pep_tcr_set[2]),
                                  batch_size, shuffle=False, num_workers=0)
            val_pep_tcr \
                = Data.DataLoader(PEP_TCR_DataSet(val_pep_tcr_set[0], val_pep_tcr_set[1], val_pep_tcr_set[2]),
                                  batch_size, shuffle=False, num_workers=0)

            model \
                = Transformer(vocab_size, n_enc_layers=n_layers, n_enc_heads=n_heads, n_dec_layers=n_layers,
                              n_dec_heads=n_heads, hla_pep_concat_len=hla_pep_concat_len,
            ...
            for epoch in range(1, epochs + 1):
                _, _, _ \
                    = model.train_per_epoch(train_pep_tcr, epoch, epochs, criterion, optimizer, num_group=2,
                                            threshold=threshold, metrics_print_=True)

                _, _, metrics_val \
                    = model.eval_per_epoch(val_pep_tcr, epoch, epochs, criterion, num_group=2, threshold=threshold,
                                           metrics_print_=True)
...
for n_layers in range(1, 10):
    for n_heads in range(1, 6):
        for fold in range(5):
            ...
            model = Transformer(vocab_size, n_enc_layers=n_layers, n_enc_heads=n_heads, n_dec_layers=n_layers,
                                n_dec_heads=n_heads, hla_pep_concat_len=hla_pep_concat_len,
                                pep_tcr_concat_len=pep_tcr_concat_len,device=device).to(device)
            ...
            for epoch in range(1, epochs + 1):
                _, _, _ \
                    = model.train_per_epoch(train_loader, epoch, epochs, criterion, optimizer, num_group=1,
                                            threshold=threshold, metrics_print_=True)

                _, _, metrics_val \
                    = model.eval_per_epoch(val_loader, epoch, epochs, criterion, num_group=1, threshold=threshold,
                                           metrics_print_=True)
